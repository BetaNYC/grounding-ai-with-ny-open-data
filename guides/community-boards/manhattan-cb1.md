...
> - 🚨 **NOT AS OF 2026-07-21 21:00 — Checkbook is returning HTTP 403.** The structured `search_spending` endpoint is blocked, not just `smart_search`, verified with and without a payee filter. **Act 3's second half will fail live until this clears.** Re-run before the meeting; if it still 403s, end Act 3 on the budget half, which is fully verified. Note the failure shape: `total_records: 0` and `records: []` are returned *alongside* an `error` field, so an agent that reads only the count will say "no payments found" rather than "the source is unavailable."
