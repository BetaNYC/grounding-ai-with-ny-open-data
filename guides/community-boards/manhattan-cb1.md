...
> - **Checkbook half — corrected and upgraded 2026-07-21.** The original note here said the Checkbook lookup was "flaky" because the endpoint was WAF-protected. That is half right, and the distinction matters: `smart_search` **is** blocked by an Incapsula WAF JavaScript challenge and should not be demoed. But the *structured* tool works fine — the catch is the parameter name. Use `search_spending` with **`payee_name`**. (`vendor` is not a declared parameter on either tool. As of `nyc-checkbook-mcp` **1.4.0** it is rejected rather than silently dropped — but on `search_spending` the refusal is zod's bare "Unrecognized key(s) in object", with no pointer to `payee_name`. The helpful "use `vendor_name`" guidance belongs to **`search_contracts`**, a different tool, and `search_spending` has no `vendor_name` parameter at all. The "millions of unrelated rows" figure was also `search_contracts`; `search_spending` requires `fiscal_year` or `issue_date_from`, so a dropped `vendor` there returned a whole fiscal year, not millions. Corrected 2026-07-21. Confirm your version — a stale `npx` cache can still serve the old build, where the silent drop is live.) `search_contracts` genuinely cannot filter by vendor name — but its declared `vendor_name` parameter returns an explicit error explaining why and listing alternatives, which is correct behavior rather than a fault. So the Checkbook half **can** be the climax now.
> - 🚨 **NOT AS OF 2026-07-21 21:00 — Checkbook returns HTTP 403 to the MCP.** The structured `/api` endpoint is blocked, not just `smart_search`. **Diagnosed:** the homepage 403s to `curl` from the same machine where it loads normally in a browser, and spoofing a Chrome User-Agent changes nothing — Incapsula's JavaScript challenge, site-wide. Not an outage, and not something we will work around. **Assume Act 3's second half stays broken.**
>   - **Fallback:** keep [checkbooknyc.com](https://www.checkbooknyc.com) open and pull the payment up by hand, or end Act 3 on the budget half, which is fully verified. This script already tells you to keep that tab open.
>   - **Failure shape to know:** `total_records: 0` and `records: []` come back *alongside* an `error` field, so an agent reading only the count will say "no payments found" rather than "the source is unavailable." Filed as [nyc-checkbook-mcp#21](https://github.com/BetaNYC/nyc-checkbook-mcp/issues/21).
